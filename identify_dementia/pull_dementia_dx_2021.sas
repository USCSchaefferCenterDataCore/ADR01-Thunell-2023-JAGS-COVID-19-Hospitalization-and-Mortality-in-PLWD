...
%getdx(bcarrier,&clmbyear,&clmeyear,dxv=prncpal_dgns_cd,dropv=,keepv=clm_id rfr_physn_npi rfr_physn_upin,byvar=clm_id);

%getdx(hha,&clmbyear,&clmeyear,dxv=prncpal_dgns_cd,dropv=,
			 keepv=at_physn_npi at_physn_upin);		
%appenddx(hha);
		
%getdx(inpatient,&clmbyear,&clmeyear,dxv=prncpal_dgns_cd,dropv=,
			 keepv=at_physn_npi at_physn_upin
			 			 op_physn_npi op_physn_upin
			 			 ot_physn_npi ot_physn_upin);	
%appenddx(inpatient);
		
%getdx(outpatient,&clmbyear,&clmeyear,dxv=prncpal_dgns_cd,dropv=,
			 keepv=at_physn_npi at_physn_upin
			 			 op_physn_npi op_physn_upin
			 			 ot_physn_npi ot_physn_upin);		
%appenddx(outpatient);

%getdx(snf,&clmbyear,&clmeyear,dxv=prncpal_dgns_cd,dropv=,
...
data &outlib..dementia_dx_carline_&year;
		...
		set 
			...
			%else %if &year>=&rifqyear %then %do;
				%do mo=1 %to 12;
					%if &mo<10 %then %do;
						rifq&year..bcarrier_line_0&mo (keep=bene_id clm_thru_dt line_icd_dgns_cd clm_id prf_physn_npi prf_physn_upin prvdr_spclty)
					%end;
					%if &mo>=12 %then %do;
						rifq&year..bcarrier_line_&mo (keep=bene_id clm_thru_dt line_icd_dgns_cd clm_id prf_physn_npi prf_physn_upin prvdr_spclty) 
					%end;
				%end;
			%end;
				;
