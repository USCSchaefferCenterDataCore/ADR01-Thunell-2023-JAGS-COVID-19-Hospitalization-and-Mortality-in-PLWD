...
%macro phys(byear,eyear);
		...
		data &tempwork..inpatient_&yr.;
			set 
				%if &yr.<&demogv. %then %do;
					...
					%end;
				%end;
				%if &yr.>=&demogv. %then %do mo=1 %to &rifqmo;
					%if &mo<10 %then %do;
	...
	* Setting all together;
	data &tempwork..acute_ip_2021;
		set &tempwork..inpatient_&byear.;
		by bene_id clm_id;
	run;
...
proc freq data=&tempwork..acute_ip_2021 noprint;
	table prvdr_34dig / out=&tempwork..prvdr_ck;
run;

* Checking how many claims occur on the same day;
proc sort data=&tempwork..acute_ip_2021 out=ip_s_dt nodupkey; by bene_id clm_thru_dt clm_id; run;
...
proc sort data=&tempwork..acute_ip_2021 out=ip_s_clm nodupkey; by bene_id clm_id clm_thru_dt; run;
